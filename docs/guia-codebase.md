...
|---|---|---|---|
| **Dominar** | `setup-spyder/src/setup_spyder/` (submódulo) | A codebase do produto: CLI/API pública (`cli.py`), launcher e bootstrap filho (`launcher.py`, `bootstrap.py`), perfis (`perfil.py`) e o plugin AI Terminal (`plugin/`). A suíte do plano está em `setup-spyder/tests/`. | Toda a implementação do MVP fica aqui e é distribuída na wheel `setup-spyder`; o fork não empacota nada disso. |
...
| **Validar** | `setup.py` do fork e `setup-spyder/pyproject.toml` | Entry point, dependências condicionais e inclusão dos assets locais do xterm. | Garantir que wheel/sdist funcionem; instalação editável sozinha não prova o empacotamento. |
...
| `python-lsp-server`, `qtconsole` e `spyder-kernels` | Vêm do PyPI (não há mais subrepos em `external-deps/`); não participam do transporte do terminal. | Apenas diante de falha reproduzível causada por uma dessas integrações. |
...
|---|---|---|
| argumentos, projeto, perfil ou processo-filho | `setup-spyder/src/setup_spyder/cli.py`, `launcher.py`, `bootstrap.py`, `perfil.py` | alterar `spyder/app/start.py` |
| painel, preferências ou sessão do agente | `setup-spyder/src/setup_spyder/plugin/` | adicionar um plugin interno ao `setup.py` deste fork |
...
**Linha divisória:** se uma mudança no diretório `spyder/` parece necessária para entregar o MVP, primeiro escreva um teste de integração que prove a limitação. O resultado preferido continua sendo corrigir `setup-spyder/` e manter o fork apenas como ambiente de validação.
